...
├── configurar-admin.html # Bootstrap do primeiro admin (uso único, veja Passo 8)
...
│   ├── supervisao.js
│   ├── index.js
│   └── configurar-admin.js   # Lógica da página de bootstrap do primeiro admin
...
## Passo 8 — Criar o primeiro admin (automático, só uma vez)

Não existe cadastro público nesta plataforma — nem mesmo o primeiro usuário se cadastra sozinho pela tela de login. Por isso, a primeiríssima conta (a sua, de admin) precisa de uma etapa especial de "bootstrap". Esta versão do projeto já vem com uma página que faz isso por você em um clique, sem precisar mexer no Console do Firebase.

> 📄 **Seus dados já estão prontos** no arquivo `ADMIN-INICIAL-NAO-SUBIR.txt` (nesta mesma pasta), caso queira conferi-los ou usar o caminho manual (abaixo). Esse arquivo já está no `.gitignore` para nunca ser enviado ao GitHub; depois de usá-lo, é uma boa ideia apagá-lo do seu computador ou trocar a senha pelo link "Esqueci minha senha" assim que entrar pela primeira vez.

### Caminho automático (recomendado)

1. Confirme que já concluiu os Passos 1 a 4 (projeto Firebase criado, login por e-mail/senha ativado, Firestore criado e credenciais coladas em `js/firebase-config.js`).
2. Abra `configurar-admin.html` no navegador (local, com o servidor de teste — veja "Testar no seu computador" mais abaixo — ou já publicado no GitHub Pages).
3. Confira nome, e-mail e senha (já vêm preenchidos) e clique em **Criar conta de admin**.
4. Pronto — a página cria sua conta e seu perfil de admin automaticamente, e te leva direto para o painel.

Essa página só funciona **uma vez**: assim que o primeiro admin é criado, as regras do `firestore.rules` fecham essa brecha sozinhas (ninguém mais consegue usar essa página para virar admin depois disso — inclusive você, se tentar rodar de novo, vai só receber um aviso de que a conta já existe).

**Importante:** faça esse passo logo depois de publicar o site, antes de divulgar a URL para qualquer pessoa. Enquanto o bootstrap não é concluído, alguém que descobrisse a URL do site e soubesse o e-mail configurado em `emailPrimeiroAdmin()` (dentro de `firestore.rules`) poderia tentar chegar primeiro — depois que seu admin existe, essa janela se fecha para sempre.

### Caminho manual (alternativa, caso prefira ou a página automática dê algum problema)
...
- **Bootstrap do primeiro admin:** a página `configurar-admin.html` só pode ser usada por quem se autentica com o e-mail definido em `emailPrimeiroAdmin()` (dentro de `firestore.rules`), e só enquanto nenhum admin existir ainda. Ainda assim, faça esse passo assim que publicar o site, antes de divulgar a URL — veja o aviso completo no Passo 8.
